...
class ESP8266ROM(ESPLoader):
    ...
    def read_mac(self):
        ...
        self.MAC_ADDRESS = oui + ((mac1 >> 8) & 0xff, mac1 & 0xff, (mac0 >> 24) & 0xff)
...
class ESP32ROM(ESPLoader):
    ...
    def read_mac(self):
        ...
        try:
            self.MAC_ADDRESS =  tuple(ord(b) for b in bitstring)
        except TypeError:  # Python 3, bitstring elements are already bytes
            self.MAC_ADDRESS =  tuple(bitstring)
        return self.MAC_ADDRESS
...
def main(argv=None, esp=None, return_array=[]):
    """
...
def _main(return_array=[]):
    try:
        main(return_array=return_array)
    except FatalError as e:
